	...
	public TypeNode visitNode(CallNode n) throws TypeException {
        ...
        if (!(t instanceof  ArrowTypeNode)) {
            throw new TypeException(
                "Invocation of a non-function " + n.id,
                n.getLine()
            );
        }
        if (!(((ArrowTypeNode) t).parlist.size() == n.arglist.size())) {
            throw new TypeException(
        ...
        for (Node arg : n.arglist) {
            try {
                ...
                System.out.println(
                    "Wrong type for "+ i + "-th parameter in the invocation of " + e.text
                );
            }
            i++;
        }
		return ((ArrowTypeNode) t).ret;
	}
